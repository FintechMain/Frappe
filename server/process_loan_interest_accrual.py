"""
Process Loan Interest Accrual API Module
...
@process_loan_interest_accrual_bp.route('/api/process-loan-interest-accrual', methods=['POST'])
def create_process_loan_interest_accrual():
    """Create a new process loan interest accrual"""
...
@process_loan_interest_accrual_bp.route('/api/process-loan-interest-accrual', methods=['GET'])
def get_process_loan_interest_accrual():
    """Get list of process loan interest accrual records"""
    params = {}
...
@process_loan_interest_accrual_bp.route('/api/process-loan-interest-accrual/<process_name>', methods=['PUT'])
def update_process_loan_interest_accrual(process_name: str):
    """Update a process loan interest accrual"""
    data = request.get_json()
    if not data:
        return jsonify({'error': 'Request body is required'}), 400
    endpoint = f'{BASE_ENDPOINT}/{process_name}'
