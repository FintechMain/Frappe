"""
Loan Repayments API Module
Handles all CRUD operations for Loan Repayments
"""
from flask import Blueprint, request, jsonify
from utils import make_frappe_request, url_encode_doctype

loan_repayments_bp = Blueprint('loan_repayments', __name__)
DOCTYPE = "Loan Repayment"
ENCODED_DOCTYPE = url_encode_doctype(DOCTYPE)
BASE_ENDPOINT = f'/api/resource/{ENCODED_DOCTYPE}'


@loan_repayments_bp.route('/api/loan-repayments', methods=['POST'])
...
@loan_repayments_bp.route('/api/loan-repayments', methods=['GET'])
def get_loan_repayments():
    """Get list of loan repayments"""
    params = {}
...
@loan_repayments_bp.route('/api/loan-repayments/<repayment_name>', methods=['GET'])
def get_loan_repayment(repayment_name: str):
    """Get a specific loan repayment"""
    endpoint = f'{BASE_ENDPOINT}/{repayment_name}'
    status_code, response_data = make_frappe_request('GET', endpoint)
    return jsonify(response_data), status_code


@loan_repayments_bp.route('/api/loan-repayments/<repayment_name>', methods=['PUT'])
def update_loan_repayment(repayment_name: str):
...
@loan_repayments_bp.route('/api/loan-repayments/<repayment_name>', methods=['DELETE'])
def delete_loan_repayment(repayment_name: str):
